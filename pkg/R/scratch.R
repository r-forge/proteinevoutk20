optr.w <- function(beta,gamma,generange,tree,multicore=FALSE,...){
  ...
  opts <- list("algorithm"="NLOPT_LN_SBPLX","maxeval"="1000000","xtol_rel"=1e-7,"ftol_rel"=.Machine$double.eps^0.5,"print_level"=1)
...
optr.s.weight <- function(data, tree, s,beta,gamma, ...){
  ...
  upper <- rep(Inf,3)
  #options for optimizer
  opts <- list("algorithm"="NLOPT_LN_COBYLA","maxeval"="1000000","xtol_rel"=1e-7,"ftol_rel"=.Machine$double.eps^0.5,"print_level"=1)
...
optimQ <- function(tree,data,Q=rep(1,6), ...){
  ...
  upper=rep(Inf,5)
  #options for optimizer
  opts <- list("algorithm"="NLOPT_LN_PRAXIS","maxeval"="1000000","xtol_rel"=1e-7,"ftol_rel"=.Machine$double.eps^0.5,"print_level"=1)
  res = nloptr(x0=ab,eval_f=fn, lb=lower,ub=upper,opts=opts,data=data,tree=tree)
...
optimBr <- function(data,tree,el=NULL,stopval=-Inf,method="NLOPT_LN_SBPLX", ...){
  ...
  #options for optimizer
  opts <- list("algorithm"=method,"maxeval"="1000000","xtol_rel"=1e-5,"ftol_rel"=.Machine$double.eps^0.5,
               "stopval"=stopval,"print_level"=1)
