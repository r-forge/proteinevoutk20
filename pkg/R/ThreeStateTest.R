...
#s: seleciton coeffecient VECTOR, (comes from a probability distribution, e.g. exponential or gamma)
...
#mu: mutation rate from one aa to another, they are assumed to be the same across
#all amino acids.
mat_gen_indep_m <- function(aa_op,s,formula=1,model=1,a1=2, a2=1, Phi=0.5,q=4*10^(-3),Ne=1.37*10^3,mu=1/(2*Ne)){
  mat <- matrix(0,nrow=m,ncol=m) #set diagonal entries to be 0 at first
  for(i in 1:(m-1)){
    for(j in (i+1):m){
      d1 <- GM[i,aa_op] #distance between aa_i and aa_op
      d2 <- GM[j,aa_op]
      mat[i,j] <- 2*Ne*mu*fix(d1,d2,s,formula,model,a1,a2,Phi,q,Ne) #fixation prob -> transition rate
      mat[j,i] <- 2*Ne*mu*fix(d2,d1,s,formula,model,a1,a2,Phi,q,Ne) #symmetric entry
    }#end for j
...
#arr: the protein sequences; each row represents a protein considered in the rate matrix
#op: optimal amino acid sequences
#s: selection coefficients - a vector
#formula, model
#########################################################################3
# mat_gen_dep <- function(arr,protein_op,s,formula=1,model=1,a1=2, a2=1, Phi=0.5,q=4*10^(-3),Ne=1.37*10^3, mu=1/(2*Ne)){
...
#   for(i in 1:(n1-1)){
#     for(j in (i+1):n1){
#       d1 <- pchem_d(arr[i,],protein_op)
#       d2 <- pchem_d(arr[j,],protein_op)
#       mat[i,j] <- 2*Ne*mu*fix(d1,d2,s,formula,model,a1,a2,Phi,q,Ne) #fixation prob -> transition rate
#       mat[j,i] <- 2*Ne*mu*fix(d2,d1,s,formula,model,a1,a2,Phi,q,Ne) #symmetric entry
#     }
#     mat[i,i] <- -sum(mat[i,])
...
#If the length of the protein is l, then it has 19*l neighbors
rate_move_m <- function(protein, protein_op, s,indep=TRUE,formula=1,model=1,a1=2, a2=1, Phi=0.5,q=4*10^(-3),Ne=1.37*10^3,mu=1/(2*Ne)){
  l <- length(protein)
  if((length(s)!=l)&&(length(s)==1)){ #if s is given as a number but protein has more than 1 sites
    sv <- rep(s,l) #consider s to be the same across all sites
  }
  else if(length(s)==l){
    sv <- s
  }
  rates <- vector() # rates of moving to neighboring proteins
  ...
  for(i in 1:l){ #loop over every position
    ...
    for(j in 1:(m-1)){ #loop over the m-1 amino acids
        ...
        if(indep==TRUE){ #site-independent case
          rates <- c(rates,2*Ne*mu*fix(d1[i],d2[i],sv[i],formula,model,a1,a2,Phi,q,Ne))
        }
        else{ #site-dependent case
          rates <- c(rates,2*Ne*mu*fix(d1,d2,sv,formula,model,a1,a2,Phi,q,Ne))
        }
    }
  }
  rates
...
simulation_m<- function(protein,protein_op,t,s=1,indep=TRUE,formula=1,model=1,a1=2, a2=1, Phi=0.5,q=4*10^(-3),Ne=1.37*10^3,mu=1/(2*Ne),record.fitness=TRUE){
  #browser()
  ...
  while(t_now < t){ #when current time is less than t
    rates <- rate_move_m(protein,protein_op,s,indep,formula,model,a1,a2,Phi,q,Ne,mu) #moving rates of a protein to its neighbors
...
find_freq <- function(sim, sites,transitions,Ne=1.37e3){
  browser()
