...
lm.est.vals <- matrix(ncol=2, nrow=0)
lm.true.vals <- matrix(ncol=2, nrow=0)
for (rep in sequence(nreps)) {
  ...
  lm.est <- lm(est.logphig~est.logphi)
  lm.true <- lm(est.logphig~true.logphi)
  lm.est.vals <- rbind(lm.est.vals,as.numeric(lm.est$coefficients))
  lm.true.vals <- rbind(lm.true.vals,as.numeric(lm.true$coefficients))
  print(lm.est$coefficients)
  print(lm.true$coefficients)
}
par(mfcol=c(1,3))
