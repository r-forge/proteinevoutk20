...
##simulation of a Markov chain, given the substitution rate matrix
## the starting state and the simulating time of the chain
sim_simple <- function(start_state,matrix,t){
  ...
  while(t_now < t){
    ...
    if(t_now < t){
      ...
      states <- c(1:l)[-start_state]
      start_state <- sample(states,1,replace=T,rates)
...
for(i in 1:length(data)){
  ...
  resample <- vector(mode="character",length=1701) #store the resampled result
  for(j in 1:l_aa){
    resample[((j-1)*3+1):(j*3)] <- d[((aa_order[j]-1)*3+1):(aa_order[j]*3)]
  }
  data_resample[[i]] <- resample #substitute old data with new ones
}
...
<<<<<<< .mine
file <- paste("rokas_btsp",c(1:12,31:74),".RData",sep="")
l <- length(file)
btsp_arr <- array(dim=c(l,3))
for(i in 1:l){
  load(file[i])
  btsp_arr[i,] <- mle[[1]]$par
}=======
load("rokas_sw.RData")
load("mutation.RData")
m <- attr(mle,"details")
...
para_data <- simTree(tree,500,protein_op=protein_op,20,s,Nu_vec,alpha=1,beta,gamma,q=4e-7,Ne=1.36e7)

f <- function(x,s,p){
  l <- length(x)
  res <- vector(length=l)
  res <- 2*x*p*(1-exp(-2*x*p*s))/(1-exp(-2*x*s))
  return(res)
}>>>>>>> .r22
